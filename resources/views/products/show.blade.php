...
@section('content')
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav class="flex items-center gap-2 text-[12px] mb-6 flex-wrap" style="color: var(--text-tertiary);">
            ...
            <span>/</span>
            <a href="{{ route('products.index') }}" class="transition-colors duration-150 hover:text-[color:var(--gold)]">المنتجات</a>
            <span>/</span>
        ...
        <div class="grid md:grid-cols-2 gap-10 mb-14">
            ...
            <div>
                <div class="aspect-square rounded-xl overflow-hidden mb-3 zoom-container" style="background-color: var(--bg-tertiary);">
                    ...
                    <div class="grid grid-cols-5 gap-2">
                        @foreach($product->images as $image)
                            <button onclick="document.getElementById('mainImage').src='{{ asset('storage/' . $image->image_path) }}'"
                                    class="aspect-square rounded-lg overflow-hidden border-2 transition-all duration-150"
            ...
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-bold mb-4 leading-tight" style="color: var(--text-primary);">{{ $product->name }}</h1>
                ...
                <div class="flex flex-wrap gap-2 mb-5">
                    @auth
                        ...
                        <button type="button" data-wishlist-btn data-product-id="{{ $product->id }}"
                            ...
                            <svg class="w-4 h-4 wishlist-icon" fill="{{ $inWishlist ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="relative inline-block" x-data="{ shareOpen: false }">
                        ...
                        <div x-show="shareOpen" @click.away="shareOpen = false" x-cloak
                            ...
                            <p class="px-4 py-2 text-[11px] font-semibold border-b" style="color: var(--text-tertiary); border-color: var(--border-light);">
                                شارك المنتج عبر
                            </p>

                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('products.show', $product)) }}"
                               target="_blank" class="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors">
                            ...
                            <button type="button" onclick="copyProductLink(this)"
                                ...
                                <div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background-color: var(--gold-soft);">
                                    <svg class="w-3.5 h-3.5" style="color: var(--gold);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-2 mb-6">
                    ...
                    <span class="text-[12px]" style="color: var(--text-tertiary);">({{ $product->reviews->count() }} مراجعة)</span>
                </div>
                ...
                @if($totalStock > 0 && $totalStock <= 5)
                    <div class="rounded-xl p-3.5 mb-5 flex items-center gap-3 border" style="background-color: #fef2f2; border-color: #fecaca;">
                ...
                <div class="flex items-baseline gap-3 mb-5">
                    @if($product->discount_price)
                        <span class="text-3xl font-bold" style="color: var(--gold);">${{ number_format($product->discount_price, 2) }}</span>
                        <span class="text-lg line-through" style="color: var(--text-tertiary);">${{ number_format($product->price, 2) }}</span>
                        <span class="text-[11px] font-bold px-2 py-0.5 rounded" style="background-color: #fee2e2; color: #dc2626;">
                            {{ round((($product->price - $product->discount_price) / $product->price) * 100) }}-%
                        </span>
                    @else
                        <span class="text-3xl font-bold" style="color: var(--gold);">${{ number_format($product->price, 2) }}</span>
                    @endif
                </div>

                @if($product->description)
                    <p class="text-[14px] leading-relaxed mb-6" style="color: var(--text-secondary);">{{ $product->description }}</p>
                @endif

                @if(session('success'))
                    <div class="px-4 py-3 rounded-lg mb-4 text-[13px]" style="background-color: #f0fdf4; color: #166534; border: 1px solid #bbf7d0;">
                        {{ session('success') }}
                    </div>
                @endif

                @if(session('error'))
                    <div class="px-4 py-3 rounded-lg mb-4 text-[13px]" style="background-color: #fef2f2; color: #991b1b; border: 1px solid #fecaca;">
                        {{ session('error') }}
                    </div>
                @endif
                ...
                @if($product->variants->count())
                    <form action="{{ route('cart.add') }}" method="POST" class="mb-6" id="addToCartForm">
                        ...
                        @if(count($uniqueColorsData) > 1 || (count($uniqueColorsData) === 1 && $uniqueColorsData[0]['color'] !== 'بدون لون'))
                            <div class="mb-5">
                                <label class="block text-[12px] font-semibold mb-3" style="color: var(--text-primary);">
                                    اللون:
                                    <span id="selectedColorName" style="color: var(--gold); font-weight: 400;">{{ $uniqueColorsData[0]['color'] ?? '' }}</span>
                                </label>
                                <div class="flex flex-wrap gap-2" id="colorOptions">
                                    @foreach($uniqueColorsData as $index => $colorData)
                                        <button type="button" onclick="selectColor({{ $index }})"
                                                data-color-index="{{ $index }}"
                                                data-color="{{ $colorData['color'] }}"
                                                title="{{ $colorData['color'] }}"
                                                class="color-circle w-10 h-10 rounded-full border-2 transition-all duration-150 relative
                                                       {{ $index === 0 ? 'scale-110' : '' }}"
                                            ...
                                            @if(in_array(strtolower($colorData['hex']), ['#ffffff', '#f5f5dc', '#c0c0c0']))
                                                <span class="absolute inset-0 rounded-full border" style="border-color: var(--border-medium);"></span>
                                            @endif
                                        </button>
                                    @endforeach
                                </div>
                        ...
                        <div id="variantError" class="hidden px-4 py-2 rounded-lg mb-4 text-[12px]" style="background-color: #fef2f2; color: #991b1b; border: 1px solid #fecaca;">
                            الرجاء اختيار اللون والمقاس أولاً
                        </div>

                        <div class="flex gap-2 mt-5">
                            <div class="flex items-center border rounded-lg h-11" style="border-color: var(--border-light); background-color: var(--bg-primary);">
                                <button type="button" onclick="decrementQty()" class="px-3 h-full font-bold transition-colors" style="color: var(--text-secondary);">−</button>
                                <input type="number" name="quantity" id="qtyInput" value="1" min="1"
                                       class="w-12 text-center border-0 focus:outline-none font-semibold text-[13px] bg-transparent" style="color: var(--text-primary);">
                                <button type="button" onclick="incrementQty()" class="px-3 h-full font-bold transition-colors" style="color: var(--text-secondary);">+</button>
                            </div>
                ...
                @else
                    <div class="px-4 py-3 rounded-lg mb-4 text-[13px]" style="background-color: #fffbeb; color: #92400e; border: 1px solid #fde68a;">
            ...
            <div class="grid md:grid-cols-3 gap-5">
                ...
                <div class="md:col-span-1">
                    <div class="rounded-xl border p-5" style="background-color: var(--bg-primary); border-color: var(--border-light);">
                        ...
                        <div class="text-center mb-5">
                            <div class="text-4xl font-bold mb-2" style="color: var(--gold);">{{ number_format($avg, 1) }}</div>
                        ...
                        <div class="space-y-2">
                            @for($i = 5; $i >= 1; $i--)
                                ...
                                <div class="flex items-center gap-2 text-[12px]">
                                    ...
                                    <div class="flex-1 rounded-full h-1.5 overflow-hidden" style="background-color: var(--bg-tertiary);">
                                        <div class="h-1.5 rounded-full transition-all" style="width: {{ $percent }}%; background-color: var(--gold);"></div>
                                    </div>
                        ...
                        @auth
                            ...
                            @else
                                <div class="mt-5 px-3 py-2.5 rounded-lg text-[12px] text-center" style="background-color: #f0fdf4; color: #166534;">
                                    تم تقييم هذا المنتج
                                </div>
                            @endif
                        @else
                ...
                <div class="md:col-span-2">
                    ...
                    @auth
                        ...
                        @endphp
                        @if(!$userReviewed)
                            <div id="reviewForm" class="hidden rounded-xl border-2 p-5 mb-5" style="background-color: var(--bg-primary); border-color: var(--gold);">
                                ...
                                <form onsubmit="submitReview(event)" id="reviewFormElement">
                                    ...
                                    <div class="mb-4">
                                        <label class="block text-[12px] font-semibold mb-2" style="color: var(--text-primary);">تقييمك</label>
                    ...
                    <div id="reviewsList" class="space-y-3">
                        @forelse($product->reviews as $review)
                            <div class="rounded-xl border p-5 review-item" style="background-color: var(--bg-primary); border-color: var(--border-light);">
                                <div class="flex items-start gap-3 mb-3">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between gap-3 flex-wrap">
                                            <div>
                                                <p class="text-[13px] font-semibold" style="color: var(--text-primary);">{{ $review->user->name ?? 'مستخدم' }}</p>
                                                <p class="text-[11px]" style="color: var(--text-tertiary);">{{ $review->created_at->format('Y-m-d') }}</p>
                                            </div>
                                            <div class="flex gap-0.5" style="color: var(--gold);">
                                                @for($i = 1; $i <= 5; $i++)
                                ...
                                @if($review->comment)
                                    <p class="text-[13px] leading-relaxed" style="color: var(--text-secondary);">{{ $review->comment }}</p>
                                @endif
                            </div>
                        @empty
                            <div class="rounded-xl border p-10 text-center" style="background-color: var(--bg-primary); border-color: var(--border-light);">
                                <div class="w-12 h-12 rounded-lg flex items-center justify-center mx-auto mb-3" style="background-color: var(--bg-tertiary);">
                                ...
                                <p class="text-[13px]" style="color: var(--text-secondary);">لا توجد تقييمات بعد. كن أول من يقيّم هذا المنتج</p>
                            </div>
                        @endforelse
                    </div>
                </div>
    ...
    @if($product->sizeGuide && $product->sizeGuide->is_active)
        <div id="sizeGuideModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
            ...
            <div class="rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl"
                ...
                <div class="sticky top-0 p-5 flex items-center justify-between border-b rounded-t-xl" style="background-color: var(--bg-primary); border-color: var(--border-light);">
                    <div>
                        <h3 class="text-lg font-bold" style="color: var(--text-primary);">{{ $product->sizeGuide->name }}</h3>
                        @if($product->sizeGuide->description)
                            <p class="text-[12px] mt-1" style="color: var(--text-secondary);">{{ $product->sizeGuide->description }}</p>
                        @endif
                    </div>
                ...
                <div class="p-5">
                    @if($product->sizeGuide->items->count())
                                    ...
                                    @foreach($product->sizeGuide->items as $item)
                                        <tr class="border-t" style="border-color: var(--border-light);">
                                            <td class="p-3 text-right">
                                                <span class="text-white px-2.5 py-1 rounded text-[11px] font-bold" style="background-color: var(--gold);">
                                                    {{ $item->size }}
                                                </span>
                                            </td>
                                            <td class="p-3 text-center" style="color: var(--text-secondary);">{{ $item->chest ?? '—' }}</td>
                                            <td class="p-3 text-center" style="color: var(--text-secondary);">{{ $item->waist ?? '—' }}</td>
                                            <td class="p-3 text-center" style="color: var(--text-secondary);">{{ $item->hips ?? '—' }}</td>
                                            <td class="p-3 text-center" style="color: var(--text-secondary);">{{ $item->length ?? '—' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="mt-5 rounded-lg p-3.5 text-[12px]" style="background-color: var(--gold-soft); color: var(--text-secondary);">
                            قِس نفسك بشريط قياس ناعم، وضع الشريط بشكل مريح على الجسم دون شد زائد.
                        </div>
                    @else
                        <p class="text-center py-8 text-[13px]" style="color: var(--text-tertiary);">لا توجد تفاصيل مقاسات لهذا الدليل.</p>
                    @endif
                </div>
            </div>
        </div>
